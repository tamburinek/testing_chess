...
import static cz.cvut.fel.pjv.chess.chessgame.board.Board.*;

/**
 * Start menu class is GUI representing first menu which pops up when code is run
 * @author artomnorba
 */
...
public class StartMenu implements Runnable {
    ...
    /**
     * Logger used when players chooses their name or when there is error in finding required file
     */
    ...
    public void run() {
        ...
        instr.addActionListener(e -> JOptionPane.showMessageDialog(startWindow,
                "If you want to start a game write names\n" +
                "next to the pieces of the pawn. Choose time and \n" +
                "click Start. Setting the timer to all\n" +
                "zeroes will start a game without time border.",
                "Instructions",
        ...
        start.addActionListener(e -> {
            ...
            blackName = bn;
            LOGGER.info("black player name is " + bn);
            setBlackIsComputer(bn.equals("Computer"));
            String wn = whiteInput.getText();
            whiteName = wn;
            LOGGER.info("white player name is " + wn);
